-- Add ticket_design_config field to events table
-- This field will store the custom ticket design configuration as JSONB

-- Add the ticket_design_config column to the events table
ALTER TABLE events 
ADD COLUMN IF NOT EXISTS ticket_design_config JSONB DEFAULT NULL;

-- Add a comment to document the field
COMMENT ON COLUMN events.ticket_design_config IS 'Custom ticket design configuration including colors, fonts, layout, and positioning settings';

-- Create an index on the ticket_design_config field for better query performance
CREATE INDEX IF NOT EXISTS idx_events_ticket_design_config 
ON events USING GIN (ticket_design_config);

-- Update the create_event_with_details function to include ticket_design_config parameter
DROP FUNCTION IF EXISTS create_event_with_details(UUID, TEXT, TEXT, DATE, TIME, TEXT, TEXT, TEXT, TEXT[], TEXT, TEXT, TEXT, JSONB, UUID, BOOLEAN, TEXT, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB, JSONB);
...
CREATE OR REPLACE FUNCTION create_event_with_details(
    ...
    p_ticket_design_config JSONB DEFAULT NULL,
...
BEGIN
    -- Create the event with status and ticket design config
    INSERT INTO events (
        ...
        audience_type, event_visibility, status, published_at, ticket_design_config
    ) VALUES (
