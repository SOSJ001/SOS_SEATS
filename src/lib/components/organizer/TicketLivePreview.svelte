<script>
  // @ts-nocheck
  /** HI-FI live ticket preview (Centre 574:440; Left 571:112; Right mirror). Fixed 420px (Figma 571:110). */
  ...
  /** When set, renders a real QR; otherwise decorative placeholder. */
  export let qrData = "";
  ...
  $: if (qrData && qrData !== lastQrData) {
    lastQrData = qrData;
    loadQr(qrData);
  } else if (!qrData && lastQrData) {
    lastQrData = "";
    qrUrl = "";
  }

  async function loadQr(data) {
    qrUrl = "";
    try {
      qrUrl = await generateQrImage(String(data));
    } catch {
      qrUrl = "";
    }
  }
...
<div
  class="relative w-[420px] max-w-full shrink-0 overflow-hidden rounded-2xl border border-paper-border shadow-[0_12px_32px_-12px_rgba(0,0,0,0.1)]"
  style="font-family: {fontCss}; background: linear-gradient(to right, #1a1a2e, #16213e 50%, #0f3460);"
  data-node-id="571:110"
>
  ...
  >
    <div class="flex items-center justify-between gap-2">
      <div class="flex items-center gap-2">
        ...
        ></span>
        <span
          data-ticket-logo-text
          class="inline-flex h-[22px] items-center text-lg font-extrabold leading-none tracking-tight text-white"
        >
          SOS SEATS
        </span>
      </div>
      <span
        data-ticket-badge
        class="inline-flex h-[22px] items-center justify-center rounded-full px-2.5 py-0 text-[9px] font-bold uppercase leading-none tracking-[1px] text-white"
        style="background-color: {accent}; -webkit-font-smoothing: antialiased;"
      >
        {badge}
      </span>
    </div>
      ...
      >
        ...
        >
          {#if qrUrl}
            ...
            />
          {:else}
            <div class="relative size-[60px] overflow-hidden bg-white" aria-hidden="true">
        ...
        <div class="flex min-w-0 flex-1 flex-col items-start gap-1.5">
          ...
          {#if layout.includeVenue && eventLocation}
            <div
              data-ticket-meta-row
              class="flex items-center gap-1 text-[10px] leading-none text-[#9ca3a8]"
            >
              <MapPin size={10} strokeWidth={2.5} color={accent} aria-hidden="true" />
              <span data-ticket-meta-text class="leading-none">{eventLocation}</span>
            </div>
          {/if}
          {#if layout.includeDateTime && dateLabel}
            ...
            >
              ...
              <span data-ticket-meta-text class="leading-none">{dateLabel}</span>
            </div>
          {/if}
        </div>
      </div>
    {:else}
      <div class="flex flex-col items-center gap-3">
        ...
        {#if layout.includeVenue && eventLocation}
          <div
            data-ticket-meta-row
            class="flex items-center gap-1.5 text-[11px] leading-none text-white/90"
          >
            <MapPin size={10} strokeWidth={2.5} color={accent} aria-hidden="true" />
            <span data-ticket-meta-text class="leading-none">{eventLocation}</span>
          </div>
