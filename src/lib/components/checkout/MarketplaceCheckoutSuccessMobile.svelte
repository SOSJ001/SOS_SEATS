<script>
  // @ts-nocheck
  /** Mobile checkout success — HI-FI checkout-success 281:77. */
  ...
  $: tierLine = [ticketTier, amountLabel].filter(Boolean).join(" · ");
  ...
  <div class="flex w-full flex-col items-center gap-5 p-6">
    ...
    >
      <div class="flex w-full flex-col gap-1">
        <h1 class="m-0 text-[22px] font-extrabold text-ink">
          Payment Confirmed
        </h1>
        <p class="m-0 text-[13px] text-ink-secondary">
          Your tickets have been reserved and secured.
        </p>
      </div>
      ...
      >
        ...
        <div class="flex items-start justify-between gap-3">
          ...
          <span class="text-right text-ink">{timestamp}</span>
        </div>
    ...
    >
      <p class="m-0 text-sm font-extrabold text-ink">Your ticket is ready!</p>
      <p class="m-0 text-[13px] text-ink-secondary">
    ...
    >
      <div class="flex min-w-0 flex-1 flex-col gap-2.5">
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span class="text-[11px] font-bold text-ink-secondary">{tierLine}</span>
        </div>
      ...
      >
        <span
          class="size-10 rounded bg-paper-border"
          aria-hidden="true"
        ></span>
    ...
    <button
      type="button"
      class="flex h-12 w-full cursor-pointer items-center justify-center rounded-xl border border-paper-border bg-white text-sm font-extrabold text-ink hover:border-brand hover:text-brand"
