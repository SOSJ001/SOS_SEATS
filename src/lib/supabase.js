...
import { supabase, ANONYMOUS_KEY } from "$lib/supabase/client.js";

export { supabase, ANONYMOUS_KEY };
export {
  updateEventWithDetails,
  createEventWithDetails,
  uploadEventImageNew,
} from "$lib/services/eventCrud.js";
export { createPaidTicketOrder, claimFreeTickets } from "$lib/services/orders.ts";
...
//load all guest rows
export async function loadGuestsRows(user_id, db = supabase) {
  try {
...
export async function loadUserEventsForSelector(userId, db = supabase) {
  try {
...
export async function searchWalletAndUserName(db = supabase) {
  const response = await db.from("userandpublickey").select("*");
  return response;
}
...
export async function loadUserEvents(userId, sessionType = "traditional", db = supabase) {
  try {
...
export async function getRecentActivities(userId, db = supabase) {
  try {
    const activities = [];
...
// Load scan history for an event (check-ins from both guests and order_items)
export async function loadScanHistory(eventId, limit = 50) {
  try {
    const scanHistory = [];

    // Load check-ins from guests table
    const { data: guestsCheckIns, error: guestsError } = await supabase
      .from("guests")
      .select(
        `
        id,
    ...
    if (!guestsError && guestsCheckIns) {
      guestsCheckIns.forEach((guest) => {
    ...
    // Load check-ins from order_items table (wallet-based tickets)
    // First get order IDs for this event
    const { data: eventOrders, error: ordersError } = await supabase
      .from("orders")
      .select("id")
      .eq("event_id", eventId);

    if (!ordersError && eventOrders && eventOrders.length > 0) {
      const orderIds = eventOrders.map((o) => o.id);

      const { data: orderItemsCheckIns, error: orderItemsError } =
        await supabase
          .from("order_items")
          .select(
            `
      ...
      if (!orderItemsError && orderItemsCheckIns) {
        orderItemsCheckIns.forEach((item) => {
    ...
    // Sort all check-ins by time (most recent first) and limit
